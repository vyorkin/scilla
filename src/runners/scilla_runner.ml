...
module Prof = struct
  ...
  let main = create ~name:"Scilla_runner.main"

  let run = create ~name:"Scilla_runner.run"
  let gas_remaining = create ~name:"Scilla_runner.run/gas_remaining"

  let check_libs = create ~name:"Scilla_runner.check_libs"
  let deploy_library = create ~name:"Scilla_runner.deploy_library"
end
...
let run (cli : Cli.ioFiles) =
  ...
  | Ok cmod ->
      ...
      let res = Out_channel.with_file cli.output ~f:(fun channel ->
          if cli.pp_json then
